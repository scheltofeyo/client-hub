 ...
 * a person.
 *
 * The label is deliberately the channel ("via MCP"), not the name of whatever
 * wrote it: a reader scanning a logbook cares that a machine wrote this, not
 * which credential it used. That name — a personal API token, or a connected
 * app like Claude — is kept on the tooltip so the audit trail is still one
 * hover away, and it stays stored in `createdVia` either way.
 *
...
export default function ViaToken({ via, className = "" }: { via?: string | null; className?: string }) {
  ...
  return (
    <span
      ...
      title={`Geschreven via een koppeling ("${via}")`}
    >
      <Plug size={10} />
      via MCP
    </span>
